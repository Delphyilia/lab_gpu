...
# --- 学習時と同じ設定の読み込み ---
BATCH_SIZE = srgan_pretrain3.DEFINE["BATCH_SIZE"]
EPOCHS = srgan_pretrain3.DEFINE["EPOCHS"]
LEARNING_RATE = srgan_pretrain3.DEFINE["LEARNING_RATE"]
RESIDUAL_BLOCKS = srgan_pretrain3.DEFINE["RESIDUAL_BLOCKS"]
GAN_LOSS_WEIGHT = srgan_pretrain3.DEFINE["GAN_LOSS_WEIGHT"]
OTHERINFO = srgan_pretrain3.DEFINE["OTHERINFO"]  


suffix = OTHERINFO if OTHERINFO else "BASIC"
FILENAME = f"GAN_{BATCH_SIZE}_{EPOCHS}_{LEARNING_RATE}_{RESIDUAL_BLOCKS}_{GAN_LOSS_WEIGHT}_{suffix}"
...
# モデルの初期化
GENERATOR = srgan_pretrain3.Generator().to(DEVICE)
DISCRIMINATOR = srgan_pretrain3.Discriminator().to(DEVICE)

# LPIPSモデルの初期化 (VGGネットワークを使用)
loss_fn_vgg = lpips.LPIPS(net='vgg').to(DEVICE)

# --- データの準備 ---
full_train_dataset = srgan_pretrain3.STL10SRDataset(root='./data', split='train')
